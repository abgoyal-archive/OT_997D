...
static const char* device3 = NULL;
...
// JRD
                        device3 = strdup(dev[FAT_INDEX]);
		//PR326817 Haiwei Wang -01 add begin
		    } 
                //PR326817 Haiwei Wang -01 add end
#else
...
// JRD
        if (!strcmp(v->mount_point, "/sdcard")) {
            LOGW("failed to mount %s (%s); trying %s\n",
                 v->device2, strerror(errno), device3);
            result = mount(device3, v->mount_point, v->fs_type,
                           MS_NOATIME | MS_NODEV | MS_NODIRATIME, "");
            if (result == 0) return 0;
        }
