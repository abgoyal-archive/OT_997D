...
void phDal4Nfc_i2c_typeAReader_on(void)
{
	...
	LOGE("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
	//	Unpacked data = [83 80]
	if(PNXXX2BB(fd,pp15))
	{
		LOGE("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbb");
		goto typeA_on_failed;
		LOGE("cccccccccccccccccccccccccccccccc");
	}

	LOGE("dddddddddddddddddddddddddddd");
	unsigned char pp16[]={0x06, 0x93, 0x83, 0x50, 0x00, 0x92, 0x0F };

	//	Unpacked data = [83 80]
	if(PNXXX2BB_CHECK(fd,pp16) == -1)
       {
			LOGE("eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee");
			goto typeA_on_failed;
			LOGE("fffffffffffffffffffffffffffffffffffffffffffffff");
	}
        else 
       {
             mDetectedTagCount++;
...
typeA_on_failed:
    LOGE("hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh");
	HW_RESET(fd);
	close(fd);
}
