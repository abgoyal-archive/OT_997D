...
{
	DISP_STATUS ret = DISP_STATUS_OK;
       // PR 316827,318491,323348,324723 jipu.xiong@jrdcom.com 2012-09-04 begin
       //====================== mediatek add start==============================
       if (down_interruptible(&sem_update_screen)) {
       ...
       //====================== mediatek add end =============================
       // PR 316827,318491,323348,324723 jipu.xiong@jrdcom.com 2012-09-04 end
...
DISP_STATUS DISP_Capture_Framebuffer(unsigned int pvbuf, unsigned int bpp)
{
    DISP_STATUS ret;
    DISP_FUNC();
    // PR 342343 jipu.xiong@jrdcom.com 2012-11-14 begin
    //====================== mediatek add start============================
    if (down_interruptible(&sem_update_screen)) {
        printk("ERROR:Can't get sem_update_screen in DISP_Capture_framebuffer()\n");
        return DISP_STATUS_ERROR; 
    }
    disp_drv_init_context();
    if (disp_drv->capture_framebuffer) {
        ret = disp_drv->capture_framebuffer(pvbuf,bpp);
    } else {
        ret = DISP_STATUS_NOT_IMPLEMENTED;
    }
    up(&sem_update_screen);
    ...
    */
    //====================== mediatek add end============================
    // PR 342343 jipu.xiong@jrdcom.com 2012-11-14 end
}
